"""Temporary backfill: sync notes for deals that have fewer notes loaded than expected."""
...
def _pending_deals() -> list[dict]:
    """Find deals where actual note count < expected, using RPC."""
    print("  Finding deals with note gaps ...")
    ...
    print(f"  {len(all_deals)} deals with notes expected")
    ...
    for i in range(0, len(deal_ids), BATCH):
        ...
        result = (
            supabase.rpc("count_notes_per_deal", {"deal_ids": batch}).execute()
        )
    ...
    pending = [
        ...
        if actual_counts.get(d["id"], 0) < d["numero_de_notas"]
    ]
...
def _pending_deals_simple() -> list[dict]:
    """Fallback: load deals and count notes separately."""
    print("  Loading deals with notes ...")
    all_deals = []
    offset = 0
    while True:
        result = (
    ...
    print(f"  {len(all_deals)} deals with notes expected")
    print("  Counting loaded notes per deal ...")
    ...
    pending = [
        d for d in all_deals
        if loaded.get(d["id"], 0) < d["numero_de_notas"]
    ]
...
def main():
    ...
    for i, deal in enumerate(deals, 1):
        try:
            print(f"[{i}/{len(deals)}] deal {deal['deal_id']} ({deal['numero_de_notas']} notes)")
