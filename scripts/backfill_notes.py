...
def _pending_deals() -> list[dict]:
    print("  Loading all deals with notes ...")
    deals = []
    offset = 0
    while True:
        result = (
    ...
    print(f"  {len(deals)} deals with notes in total")

    print("  Counting notes already loaded per deal ...")
    loaded: dict[str, int] = {}
    offset = 0
    while True:
        result = (
    ...
    pending = [d for d in deals if loaded.get(d["deal_id"], 0) < d["numero_de_notas"]]
